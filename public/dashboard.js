let cardcontainer = document.querySelector('.card-container')
axios.get('/api/allSchemes').then(res=>res.data).then((data)=>{
  console.log(data);
})
for (let i = 0; i < 6; i++) {
    let bool = true
    let card = document.createElement('div');
    card.innerHTML = `<div class="card m-2" style="width: 18rem;">
    <img src="data:image/png;base64," class="card-img-top" alt="...">
    <div class="card-body">
      <h5 class="card-title">Card title</h5>
      <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
      <a href="#" class="btn btn-primary ${!bool ? 'd-none' : ''}" onclick="enrollApplication('bye')">enroll</a>
      <a href="#" class="btn btn-primary ${bool ? 'd-none' : ''}" onclick="cancelApplication('hi')" >cancel application</a>
    </div>
  </div>
  `
    cardcontainer.appendChild(card)
}

function cancelApplication(param) {

}

function enrollApplication(param) {
    let text = "are you sure you are eligible for this scheme";
    if (confirm(text) == true) {
        alert('done')
    }
}